...
function CheckoutSuccessPageContent() {
  ...
  const isAggregate = !!orderIdParam;
  const allTicketIds = isAggregate ? ticketIdsParam.split(',').filter(Boolean) : (ticketId ? [ticketId] : []);
  const primaryTicketId = allTicketIds[0] || '';
  ...
  useEffect(() => {
      ...
      const runVerify = async () => {
        if (isAggregate) {
          for (let i = 0; i < 4 && !cancelled; i++) {
            try {
              const result = await TicketService.checkOrderPaymentStatus(orderIdParam);
              if (cancelled) return;
          ...
          try {
            const data: any = await TicketService.getTicketDetails(primaryTicketId);
            if (!cancelled) {
              setQrPayload(
                buildClientTicketQrValue({
                  ticketId: data?.id || primaryTicketId,
            ...
            if (!cancelled) setQrPayload(buildClientTicketQrValue({ ticketId: primaryTicketId, eventId }));
          }
        } else if (primaryTicketId) {
          for (let i = 0; i < 4 && !cancelled; i++) {
            try {
              const result = await TicketService.checkPaymentStatus(primaryTicketId);
              if (cancelled) return;
              if (result.status === 'paid') {
                setPaymentVerified(true);
                break;
          ...
          try {
            ...
            if (!cancelled) {
              ...
              if (data?.status === 'paid' || data?.status === 'checkedIn') setPaymentVerified(true);
            }
          } catch {
            if (!cancelled) setQrPayload(buildClientTicketQrValue({ ticketId: primaryTicketId, eventId }));
          }
    ...
    setQrPayload('');
  }, [eventId, primaryTicketId, orderIdParam, isAggregate, router, allTicketIds.length]);

  const ticketCount = isAggregate ? allTicketIds.length : undefined;
              ...
              {isAggregate ? (
                <>
                  <div className="flex justify-between">
                    ...
                    <span className="font-mono font-bold text-[var(--text-primary)] select-all text-[9px]">{orderIdParam}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[var(--text-muted)]">Tickets</span>
                    <span className="text-[var(--text-primary)] font-medium">{allTicketIds.length} ticket(s)</span>
                  </div>
                </>
              ) : (
                <div className="flex justify-between">
                  <span className="text-[var(--text-muted)]">{t('ticket_id')}</span>
                  <span className="font-mono font-bold text-[var(--text-primary)] select-all">{primaryTicketId || t('no_info')}</span>
                </div>
              )}
              <div className="flex justify-between">
