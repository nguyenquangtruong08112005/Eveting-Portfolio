...
const smokeUserId = `smoke_user_${uuidv4()}`;
const smokeEventId = `evt_smoke_${uuidv4()}`;
const now = Date.now();
...
async function run() {
    ...
    // Seed parents required by FKs (migration 031+)
    console.log('\n  [Seed auth_users + events for FK integrity]');
    await query(
        `INSERT INTO auth_users (id, email, password_hash, roles, is_active, created_at, updated_at)
         VALUES ($1, $2, $3, $4, true, NOW(), NOW())
         ON CONFLICT (id) DO NOTHING`,
        [smokeUserId, `${smokeUserId}@smoke.test`, 'smoke_hash_not_for_login', ['user']]
    );
    await query(
    ...
    await query(
        `INSERT INTO event_ticket_types (
         ...
         ) VALUES ($1, $2, 'vip', 'VIP', 50000, 100, 100, 0, 0, true, $3, $3)
         ON CONFLICT (id) DO NOTHING`,
        [`${smokeEventId}:vip`, smokeEventId, nowDate]
    );
    assert('seed auth_users', true);
    assert('seed events', true);
    assert('seed event_ticket_types', true);
    ...
    // Ticket must exist before order_items.ticket_id FK (migration 031)
    await query(
        `INSERT INTO tickets (id, event_id, user_id, type, price, status, raw_data)
         VALUES ($1, $2, $3, $4, $5, $6, $7)`,
        [ticketId, smokeEventId, smokeUserId, 'vip', 50000, 'pending', '{}']
    );
    ...
    await orderRepository.createOrder({
        ...
        items: [
            {
                id: itemId,
                ticketTypeId: `${smokeEventId}:vip`,
                ticketType: 'VIP',
                eventId: smokeEventId,
                eventName: 'Test Event',
                ticketId: ticketId,
                seatId: null, // no seat row in section-based portfolio path
                quantity: 2,
                unitPrice: 50000,
                subtotal: 100000,
                totalAmount: 100000,
    ...
    assert('order.userId', order.userId === smokeUserId);
    assert('order.eventId', order.eventId === smokeEventId);
    assert('order.organizerId', order.organizerId === smokeUserId);
    ...
    assert('item.ticketTypeId = type row id', item.ticketTypeId === `${smokeEventId}:vip`);
    assert('item.ticketType = VIP', item.ticketType === 'VIP');
    assert('item.eventId matches smoke event', item.eventId === smokeEventId);
    assert('item.ticketId set', item.ticketId === ticketId);
    assert('item.seatId is null (no seat FK seed)', item.seatId == null);
    ...
    assert('ticketViaRepo.eventId preserved', ticketViaRepo.eventId === smokeEventId);
    ...
    await query('DELETE FROM events WHERE id = $1', [smokeEventId]);
    await query('DELETE FROM auth_users WHERE id = $1', [smokeUserId]);
