...
async function run() {
    ...
    // Read relational ticket type inventory
    const row1 = (await query(
        'SELECT available FROM event_ticket_types WHERE event_id = $1 AND code = $2',
        [testEventId, 'vip']
    )).rows[0];
    assert('event_ticket_types vip available is 8', row1 && Number(row1.available) === 8);
    const raw1 = (await query('SELECT raw_data FROM events WHERE id = $1', [testEventId])).rows[0];
    assert('raw_data name intact', raw1.raw_data && raw1.raw_data.name === 'Repeated Booking Test Event');
    ...
    assert('event_ticket_types vip available is 5', row2 && Number(row2.available) === 5);
    const raw2 = (await query('SELECT raw_data FROM events WHERE id = $1', [testEventId])).rows[0];
    assert('raw_data name still intact', raw2.raw_data && raw2.raw_data.name === 'Repeated Booking Test Event');
