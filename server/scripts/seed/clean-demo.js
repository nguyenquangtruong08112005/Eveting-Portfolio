...
const DEMO_EMAILS = [
  ...
  'organizer1@demo.eventing.moteo.fun',
  'organizer2@demo.eventing.moteo.fun',
  'organizer3@demo.eventing.moteo.fun',
  'organizer4@demo.eventing.moteo.fun',
  'organizer5@demo.eventing.moteo.fun',
  'attendee1@demo.eventing.moteo.fun',
  'attendee2@demo.eventing.moteo.fun',
  'attendee3@demo.eventing.moteo.fun',
  'attendee4@demo.eventing.moteo.fun',
  'attendee5@demo.eventing.moteo.fun',
  'attendee6@demo.eventing.moteo.fun',
  'attendee7@demo.eventing.moteo.fun',
  'attendee8@demo.eventing.moteo.fun',
  'attendee9@demo.eventing.moteo.fun',
  'attendee10@demo.eventing.moteo.fun',
  'admin@eventing.com',
...
async function main() {
  ...
  // 2. Tickets, Orders, Items, Analytics & related payment/ledger/check-in records
  count = await safeDelete(
    'ticket_check_ins',
    `id LIKE 'demo_%' OR ticket_id LIKE 'demo_%' OR event_id LIKE 'demo_%' OR staff_user_id LIKE 'demo_%' OR staff_user_id IN (SELECT id FROM auth_users WHERE email = ANY($1::text[])) OR ticket_id IN (SELECT id FROM tickets WHERE user_id LIKE 'demo_%' OR event_id LIKE 'demo_%' OR user_id IN (SELECT id FROM auth_users WHERE email = ANY($1::text[])))`,
    [DEMO_EMAILS]
  );
  console.log(`  · ticket_check_ins: ${count}`);
  totalDeleted += count;

  count = await safeDelete(
    'payment_attempts',
    `id LIKE 'demo_%' OR ticket_id LIKE 'demo_%' OR order_id LIKE 'demo_%' OR order_id IN (SELECT id FROM orders WHERE user_id LIKE 'demo_%' OR user_id IN (SELECT id FROM auth_users WHERE email = ANY($1::text[]))) OR ticket_id IN (SELECT id FROM tickets WHERE user_id LIKE 'demo_%' OR event_id LIKE 'demo_%' OR user_id IN (SELECT id FROM auth_users WHERE email = ANY($1::text[])))`,
    [DEMO_EMAILS]
  );
  console.log(`  · payment_attempts: ${count}`);
  totalDeleted += count;

  count = await safeDelete(
    'ledger_entries',
    `id LIKE 'demo_%' OR order_id LIKE 'demo_%' OR organizer_id LIKE 'demo_%' OR organizer_id IN (SELECT id FROM auth_users WHERE email = ANY($1::text[])) OR order_id IN (SELECT id FROM orders WHERE user_id LIKE 'demo_%' OR user_id IN (SELECT id FROM auth_users WHERE email = ANY($1::text[])))`,
    [DEMO_EMAILS]
  );
  console.log(`  · ledger_entries: ${count}`);
  totalDeleted += count;
