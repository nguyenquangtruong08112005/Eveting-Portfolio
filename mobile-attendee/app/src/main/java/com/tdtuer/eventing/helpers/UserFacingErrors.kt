...
object UserFacingErrors {
    ...
    private val CODE_IN_MESSAGE = Regex(
        """(?i)(with\s+)?code\s*[:\s]*\d{3}|HTTP\s*\d{3}|status\s*\d{3}|Server error:\s*\d+|failed with code:\s*\d+"""
    )

    /**
     * Preferred entry for Retrofit-style failures.
     */
    ...
    /**
     * Map any Throwable (including ones that already contain "code: 401") for UI display.
     */
    fun toUserMessage(throwable: Throwable?): String {
        ...
        // Retrofit HttpException via reflection-free message heuristics
        val raw = throwable.message?.trim().orEmpty()
        if (raw.isBlank()) return generic()

        // "HTTP 401" / "code: 500" style
    ...
    private fun messageForHttpCode(code: Int, actionHint: String?): String {
        ...
        return if (!actionHint.isNullOrBlank() && code !in 500..599) {
            // Prefer specific action only when it stays user-friendly
    ...
    private fun parseServerMessage(errorBody: String?): String? {
        ...
        // Prefer nested error.message, then top-level message (no Android JSONObject — unit-test friendly)
        ...
        // Plain text body — only if short and non-technical
    ...
    private fun extractJsonStringField(json: String, field: String): String? {
        ...
        // Prefer last match so nested error.message wins over other fields if ordered that way;
        // also try first message under "error" object by scanning all.
...
/** Exception whose message is already safe to show on screen. */
...
/** Extension for ViewModels: always map before putting into UI state. */
